...
export default function SlideshowItem(props: SlideshowItemProps) {
    ...
    return (
        <motion.div className={`lg:w-10/12 md:w-10/12 sm:w-2/3 flex justify-start items-center h-5/6 rounded-2xl bg-slate-600 duration-300 group relative hover:shadow-2xl overflow-hidden`}>
            ...
            <motion.div className={`w-full h-full flex justify-start px-10 py-5 rounded-2xl flex-col xl:gap-5 lg:gap-4 md:gap-4 justify-center sm:gap-1 transition ease-in duration-300 opacity-0 group-hover:bg-slate-700 group-hover:opacity-100 absolute top-0 left-0`}>
